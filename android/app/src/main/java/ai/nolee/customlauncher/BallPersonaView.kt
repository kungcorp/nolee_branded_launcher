...
class BallPersonaView(context: Context): View(context) {
    ...
    fun update(stage:Stage,seconds:Float,startedAt:LocalDateTime,emotion:PersonaEmotion,listening:Boolean,returning:Boolean,camera:Float){
    ...
    override fun onDraw(c:Canvas){
        ...
        var cx=Design.WIDTH/2+(driftX+shake*2f)*(1f-thinkingCenter)+camera*10f
        ...
        if(statusAlpha>.002f){
            c.save();c.translate(44f,Design.HEIGHT/2+30f);c.rotate(-90f)
            fill(Color.WHITE,.34f*statusAlpha);ink.typeface=listeningFont;ink.textSize=52f
            ink.setShadowLayer(24f,0f,0f,Color.argb((.30f*statusAlpha*255).toInt(),255,255,255))
            ...
            for((i,ch) in word.withIndex()){
                ...
                ink.alpha=(255*.34f*statusAlpha*reveal).toInt()
        ...
        val shadowX=Design.WIDTH/2+camera*8f
    ...
    private fun drawBody(c:Canvas,r:Float,material:Float,p:Float,ep:Float,t:Float,settled:Float){
        ...
        val reflection=-135f-gazeX*18f+gazeY*9f+tilt*.5f+shake*3f-camera*24f
        ...
        c.save();c.rotate(tilt+shake*2f-camera*6f)
        c.translate(camera*r*.14f,camera*r*.09f);c.scale(1f-camera*.095f,1f-camera*.06f)
        ...
        c.restore()
        c.save();c.translate(camera*r*.10f,camera*r*.065f)
        c.scale(1f-camera*.09f,1f-camera*.06f);c.rotate(-camera*6f)

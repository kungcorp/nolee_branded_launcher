 ...
 *
 *     adb shell pm grant ai.nolee.customlauncher android.permission.WRITE_SECURE_SETTINGS
 */
class VitalsMonitor(context: Context) {
    ...
    private suspend fun window(metric: VitalMetric) {
        ...
        val lock = runCatching { power?.newWakeLock(PowerManager.PARTIAL_WAKE_LOCK, "nolee-custom-launcher:vitals")?.apply { acquire(WINDOW_MS + 5_000L) } }.getOrNull()

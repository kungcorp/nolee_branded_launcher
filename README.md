# Peace launcher — design 01

## Native Cloud AI persona

Long-hold the watch face to open the ball persona. It records a spoken question, streams a Cloud
answer and plays the returned audio, then listens for a follow-up. Tap the persona to open its
transcript; hold it to interrupt and ask again. Leaving or pausing the app cancels recording,
the request and playback. Home's Vosk Voice Command remains a separate offline command flow.

The implementation is `android/app/src/main/java/ai/nolee/customlauncher/CloudAi.kt`. Before each
...
conversation turns and nonempty owner Profile fields. This context is sent to Nolee Cloud to answer
the question; conversation history lasts for the persona session. Both apps share the unit's
allowance. The persona uses spoken answers with Serena by default, and does not execute device
commands or enable web search. Edit the prompt/request in `CloudAi.kt` when forking the example.
...
`android/` is the watch app, `ai.nolee.customlauncher`, rebuilt natively in Jetpack Compose. It replaced a WebView wrapper around this prototype, which was held back by the watch's Chrome 79 WebView. The prototype in this folder remains the design reference; the app ports its portrait layout in the same 410 × 502 design coordinates.
...
- **Nolee Launcher grants:** root (the `su` fallback for radios and brightness), and the five companions (Camera, Files, Gallery, Phone, SMS). Strict kiosk blocks any app that is not an approved companion.
- **Kiosk:** the app as primary app, then kiosk.

One command does all of it, with AI agent access (ADB) on:
...
- **Bench hook** (debug builds only): run a phrase as if heard, to check a flow without speaking: `adb shell am broadcast -a ai.nolee.customlauncher.DEBUG_VOICE --es text "turn wi fi off"`.
